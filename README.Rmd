...
DE_stats <- read_tsv('DE_download_stats.tab', col_types = 'ccci') %>%
...
```{r plotDownloadsDE, echo = FALSE, dev='png', warning=FALSE}
## reorder by decreasing download counts
DE_stats <- DE_stats %>% 
    group_by(PackageComb) %>% 
    mutate(tmp = max(Nb_of_distinct_IPs)) %>% 
    ungroup() %>% 
...
ggplot(DE_stats, aes(x = Date, y = Nb_of_distinct_IPs, 
                      fill = as.factor(year(Date)),
                      color = as.factor(year(Date)),
                      width = days_in_month(Date))) + 
    geom_bar(stat = 'identity') +
